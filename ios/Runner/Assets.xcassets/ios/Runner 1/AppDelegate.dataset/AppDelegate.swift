...
@objc class AppDelegate: FlutterAppDelegate {
    ...
    // The token is stored individually under KEY_TOKEN_X, then the combined set is
    // rebuilt and the shield is applied immediately.
    private func setMonitoredNetwork(call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        switch network {
        case "instagram": pickerTitle = "Sélectionne Instagram"
        case "tiktok":    pickerTitle = "Sélectionne TikTok"
        default:          pickerTitle = "Sélectionne YouTube"
        }
        ...
        let finish: (Bool, Bool) -> Void = { [weak self, weak rootVC] save, animated in
            guard !resultCalled else {
                logger.debug("[Spark] finish called twice for \(network, privacy: .public) — ignoring")
                return
            }
            resultCalled = true
            logger.debug("[Spark] finish(save:\(save)) for network=\(network, privacy: .public)")

            if save {
                let tokenCount = model.selection.applicationTokens.count
                logger.debug("[Spark] selection.applicationTokens.count = \(tokenCount)")

                do {
                    // requiringSecureCoding:false — FamilyActivitySelection is Codable,
                    // its NSSecureCoding conformance is not publicly visible to Swift generics.
                    let data = try NSKeyedArchiver.archivedData(
                        withRootObject: model.selection,
                        requiringSecureCoding: false
                    )
                    logger.debug("[Spark] NSKeyedArchiver OK — \(data.count) bytes encoded")
                    ...
                    let verify = AppGroupStore.read()[tokenKey] as? Data
                    if let verify {
                        logger.debug("[Spark] AppGroupStore write OK — key=\(tokenKey, privacy: .public) size=\(verify.count)B")
                    } else {
                        logger.error("[Spark] AppGroupStore write FAILED — key=\(tokenKey, privacy: .public) not found after set")
                    }

                    self?.rebuildAndApplyMonitoredTokens()
                } catch {
                    logger.error("[Spark] NSKeyedArchiver THREW for \(network, privacy: .public) — \(error.localizedDescription, privacy: .public)")
                }
            }

            logger.debug("[Spark] Calling result(true) for \(network, privacy: .public)")
            result(true)
            self?.pickerSession = nil
            rootVC?.presentedViewController?.dismiss(animated: animated)
        }

        let pickerView = FamilyPickerView(
            model: model,
            title: pickerTitle,
            onDone:   { finish(true,  true) },
            onCancel: { finish(false, true) }
        )
    ...
    private func liftShieldForNetwork(networkId: String) {
        ...
        guard let sessionData = d[tokenKey] as? Data,
              let sessionSel  = decodeSelection(from: sessionData)
